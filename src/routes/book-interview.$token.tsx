...
function InterviewBookingPage() {
  ...
  if (bookedAt) {
    ...
    return <Shell><div className="mx-auto max-w-lg text-center"><span className="mx-auto grid size-14 place-items-center rounded-full bg-success/10 text-success"><CheckCircle2 className="size-7" /></span><h1 className="mt-4 font-display text-2xl font-bold">Your interview is booked</h1><p className="mt-2 text-muted-foreground">We look forward to speaking with you, {page.candidateFirstName}.</p><Card className="mt-6 p-5 text-left"><p className="text-xs font-semibold uppercase text-muted-foreground">Confirmed time</p><p className="mt-1 text-lg font-bold">{formatSlot(bookedAt, page.timezone)}</p><p className="text-sm text-muted-foreground">{page.slotMinutes} minutes · {page.timezone.replace("America/", "")}</p>{meetingLink && <Button className="mt-4 w-full" asChild><a href={meetingLink} target="_blank" rel="noreferrer">Open meeting link <ExternalLink className="size-4" /></a></Button>}</Card></div></Shell>;
  }
